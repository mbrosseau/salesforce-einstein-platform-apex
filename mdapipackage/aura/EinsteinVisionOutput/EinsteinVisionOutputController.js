({  
    onModelListUpdated: function(component, event, helper) {
       
   	 		var modelList = component.get("v.modelList");           
            if(modelList != null || modelList.length !=0 ) {
            	var modelId = modelList[0].modelId; 
                component.set("v.selectedModel", modelId); 
            }
      ...
      onDefaultModelId: function(component, event, helper) {
        ...
        onSelectedModelChange: function(component, event, helper) {
           
            var selectedModel = component.get("v.selectedModel");
            var fileData = component.get("v.fileData");
            if(fileData != null && fileData.length >0) {
                helper.analyse(component, fileData.match(/,(.*)$/)[1]);
            }
